<template>
  <div class="dashboard-container">
    ...
    <div class="metrics-grid">
      ...
      <div class="metric-card">
        <div class="metric-title">활성 시설</div>
        <div class="metric-value">{{ activeFacilities }}</div>
      </div>
      
      <div class="metric-card">
        <div class="metric-title">대기 예약</div>
          ...
          <tbody>
            <tr v-for="reservation in recentReservations" :key="reservation.id">
              ...
              <td>
                <button
                  class="detail-btn"
                  @click="$router.push(`/processor/reservations/${reservation.id}`)"
                >
                  상세보기
                </button>
              </td>
      ...
      <div class="facility-status">
        ...
        <div class="facility-list">
          ...
          >
            ...
            <div class="facility-status">
              <span :class="['status-badge', getFacilityStatusClass(facility.status)]">
...
import { processorDashboardService } from '@/services/dashboard'
...
const getFacilityStatusClass = (status) => {
  return facilityStatusClasses[status] || ''
}
...
const getFacilityStatusText = (status) => {
  return facilityStatusTexts[status] || status
}
...
const loadDashboardData = async () => {
  try {
    await withLoading(async () => {
      // 주요 지표 로드
      const metrics = await processorDashboardService.getMetrics()
      todayReservations.value = metrics.todayReservations
      weeklyReservations.value = metrics.weeklyReservations
      activeFacilities.value = metrics.activeFacilities
      pendingReservations.value = metrics.pendingReservations

      // 최근 예약 로드
      recentReservations.value = await processorDashboardService.getRecentReservations()

      // 시설 상태 로드
      facilities.value = await processorDashboardService.getFacilities()
    })
  } catch (error) {
    showError(error)
  }
