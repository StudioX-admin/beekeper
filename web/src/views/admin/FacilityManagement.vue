      ...
      <table>
        ...
        <tbody>
          <tr v-for="facility in facilities" :key="facility.id">
            <td>{{ facility.facility_number }}</td>
            <td>{{ getCompanyTypeText(facility.company_type) }}</td>
            ...
            <td>{{ formatDate(facility.last_inspection_date) }}</td>
            <td>
      ...
      <div class="dialog">
        ...
        <form @submit.prevent="submitFacility">
          <div class="form-group">
            <label>업체구분</label>
            <select v-model="facilityForm.company_type" required>
              <option value="transporter">운반업체</option>
              <option value="processor">처리업체</option>
            </select>
          </div>
          <div class="form-group">
            <label>업체명</label>
            <select v-model="facilityForm.company_id" required>
          ...
          <div class="form-group">
            ...
            <input
              type="date"
              v-model="facilityForm.last_inspection_date"
              required
            />
          </div>
          <div class="form-group">
...
import { adminFacilityService } from '@/services/facility'
...
const loadFacilities = async () => {
  try {
    await withLoading(async () => {
      ...
      const data = await adminFacilityService.getFacilities(params)
...
const loadCompanies = async () => {
  try {
    await withLoading(async () => {
      const data = await adminFacilityService.getCompanies()
...
const deleteFacility = async () => {
  try {
    await withLoading(async () => {
      await adminFacilityService.deleteFacility(selectedFacility.value.id)
...
const submitFacility = async () => {
  try {
    await withLoading(async () => {
      if (isEdit.value) {
        await adminFacilityService.updateFacility(selectedFacility.value.id, facilityForm.value)
      } else {
        await adminFacilityService.createFacility(facilityForm.value)
      }
...
// 업체구분 변경 시 업체 목록 필터링
watch(() => facilityForm.value.company_type, async (newType) => {
  if (newType) {
    await loadCompanies()
