    ...
    <div class="search-filters">
      <div class="filter-group">
        <input
          type="text"
          v-model="searchQuery"
          placeholder="차량번호, 차종으로 검색"
          @input="handleSearch"
        />
      </div>
      <div class="filter-group">
        <select v-model="companyType" @change="handleSearch">
      ...
      <div class="filter-group">
        <select v-model="statusFilter" @change="handleSearch">
          ...
          <option value="inactive">운행중지</option>
        </select>
      </div>
    </div>
      ...
      <table>
        ...
        <tbody>
          <tr v-for="vehicle in vehicles" :key="vehicle.id">
            ...
            <td>{{ getCompanyTypeText(vehicle.company_type) }}</td>
      ...
      <div class="dialog">
        ...
        <form @submit.prevent="submitVehicle">
          <div class="form-group">
            <label>업체구분</label>
            <select v-model="vehicleForm.company_type" required>
              <option value="transporter">수거업체</option>
              <option value="processor">처리업체</option>
            </select>
          </div>
          <div class="form-group">
            <label>업체</label>
            <select v-model="vehicleForm.company_id" required>
...
import { adminVehicleService } from '@/services/vehicle'
...
const loadVehicles = async () => {
  try {
    await withLoading(async () => {
      ...
      const data = await adminVehicleService.getVehicles(params)
...
const loadCompanies = async () => {
  try {
    await withLoading(async () => {
      const data = await adminCompanyService.getCompanies(vehicleForm.value.company_type)
...
const deleteVehicle = async () => {
  try {
    await withLoading(async () => {
      await adminVehicleService.deleteVehicle(selectedVehicle.value.id)
...
const submitVehicle = async () => {
  try {
    await withLoading(async () => {
      if (isEdit.value) {
        await adminVehicleService.updateVehicle(selectedVehicle.value.id, vehicleForm.value)
      } else {
        await adminVehicleService.createVehicle(vehicleForm.value)
      }
...
// 업체구분 변경 시 업체 목록 로드
watch(() => vehicleForm.value.company_type, (newType) => {
  if (newType) {
    loadCompanies()
  } else {
    companies.value = []
